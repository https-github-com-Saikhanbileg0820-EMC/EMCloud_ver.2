(function () {
    'use strict';
    kintone.events.on(
        ['app.record.edit.show', 'app.record.detail.show', 'app.record.edit.change.雇用区分', 'app.record.edit.change.雇用区分コピー', 'app.record.create.change.雇用区分', 'app.record.create.change.雇用区分コピー', 'app.record.create.show'],
        function (event) {
            const setcolmun = [
                '給与支給1',
                '給与支給2',
                    ...
                    if (resp.records[i].雇用区分.value == kubun) {
                        for (let s = 0; s < matches.length; s++) {
                            for (let p = 1; p <= setfields.length; p++) {
                                ...
                                if (matches[s].innerHTML == setfields[p] && resp['records'][i][setvals].value != '') {
                                    matches[s].innerHTML = resp['records'][i][setvals].value;
                                    matches[s].classList.add('targetform');
                                }
                            }
                        }
                    }
