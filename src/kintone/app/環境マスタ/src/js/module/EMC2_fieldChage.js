    ...
    kintone.events.on('app.record.index.show', async (e) => {
      ...
      <div id="selectHeader" style="margin-left: 20px;margin-top: 20px;height: 60px;width: 555px;background-color: #3498db;line-height: 60px;border-radius: 5px;margin-bottom: 30px;">
        ...
        const createChangeFields = (targetArray, index, type) => {
          ...
          <div class='recordColumn' style='width: 50%;display: flex;height: 50px;line-height: 50px;font-size: 12px;margin-left: 50px;'><div class='fieldName' style="width: 380px;font-size: 14px;">${targetArray[index].label} [${
                        targetArray[index].code
                    }]</div><div class='yajirusi' style="width: 50px;color: #3498db;">▶▶▶</div><input class='changeText' type='text' name='{"type":"${targetArray[index].type}","code":"${
                        targetArray[index].code
                    }"}' style="height: 30px;margin-top: 7px;width: 200px;margin-left: 50px;border: 2px solid #3498db;font-size:12px;padding-left:5px;"></div>
          <div class='recordColumn' style='width: 50%;display: flex;height: 50px;line-height: 50px;font-size: 12px;margin-left: 50px;'><div class='fieldName' style="width: 380px;font-size: 14px;">${targetArray[index + 1].label} [${
                        targetArray[index + 1].code
                    }]</div><div class='yajirusi' style="width: 50px;color: #3498db;">▶▶▶</div><input class='changeText' type='text' name='{"type":"${targetArray[index + 1].type}","code":"${
                        targetArray[index + 1].code
                    }"}' style="height: 30px;margin-top: 7px;width: 200px;margin-left: 50px;border: 2px solid #3498db;font-size:12px;padding-left:5px;"></div>
        </div>
      `)[0]
        ...
        const getShowFields = async (id, FIELD_CHANGE_MAIN, targetAppId) => {
            ...
            // 「変更反映」「キャンセル」ボタン作成
            $(FIELD_CHANGE_MAIN)[0].append(
                $(
                    `<div id="saveCancelBar" style="height: 70px;width: 100%;line-height: 70px;"><div id="btns" style="height:70px;width:400px;"><button id="saveBtn" style="height:45px;width:130px;margin-left:20px;background-color:#3498db;color:#fff;border:solid 1px #e3e7e8;">変更反映</button><button id="cancelBtn" style="height: 45px;width: 140px;margin-left: 20px;background-color: #fff;color: #3498db;border: solid 1px #e3e8e5;">キャンセル</button></div></div>`
                )[0]
            );
            for (let type in eachOtherType) {
                ...
                if (skipFieldType.indexOf(`${type}`) === -1) {
                    ...
                    $(FIELD_CHANGE_MAIN)[0].append(
                        $(
                            `<div id="${type}" class="fieldsRecord_bar" style="background-color: #3498db;cursor:pointer;height:40px;width:100%;line-height:40px;font-weight:bold;color:#fff;display:flex;border:solid 1px #fff;";><div class='clickPoint' style="width: 50px;text-align: center;">▶</div>${type}</div>`
                        )[0]
                    );
